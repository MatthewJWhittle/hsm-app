...
**Admin — update labels without re-uploading the COG:** `PATCH /projects/{project_id}/environmental-band-definitions` with a JSON **array** body (same shape as `environmental_band_definitions`, one object per band, indices `0..n-1`). Requires an existing environmental COG on the server; the band count must match the raster. Use this from scripts or curl to set display **`label`** values (and **`name`** if you need to change stable feature names).
