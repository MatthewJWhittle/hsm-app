...
3. **Interface:** The server calls **`predict_proba(X)`** on the loaded object with a **2D numpy array** of shape `(n_samples, n_features)` and uses column **`positive_class_index`** from model metadata (default **`1`**) for the positive-class probability column. Your estimator **must** implement **`predict_proba`** suitable for binary (or multi-class) probability output as sklearn classifiers do.

4. **Feature order:** Training column order must match **`metadata.analysis.feature_band_names`** on the model (same order as **`feature_names`** resolved server-side from the catalog project). The server builds a one-row **pandas** `DataFrame` with those column names before SHAP.
