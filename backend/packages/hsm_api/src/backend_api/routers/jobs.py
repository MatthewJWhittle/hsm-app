...
from hsm_core.jobs import get_job, try_abandon_stale_pending_job
...
async def get_job_status(
    ...
    abandon_after = (
        settings.job_pending_abandon_after_seconds
        if settings.job_pending_abandon_after_seconds > 0
        else None
    )
    job = try_abandon_stale_pending_job(
        client, job, abandon_after_seconds=abandon_after
    )

    uid = subject_uid_from_claims(claims)
