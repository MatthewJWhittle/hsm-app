...
- **`POST /auth/token`** — exchange **email/password** (JSON body) for Firebase **ID token** and refresh token via this API (the server calls Identity Toolkit; you do not need to curl Google’s URL). Use **`id_token`** as **`Authorization: Bearer`** on admin routes. Optional **`admin_only: true`** returns **`403`** unless the user has custom claim **`admin: true`**. In production, set **`FIREBASE_WEB_API_KEY`** to your Firebase **Web API key** (same as in the web app config); with the Auth emulator, a placeholder key is used automatically.
