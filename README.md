...
- **Prod Hosting:** **`hsm-dashboard`** — **`https://hsm-dashboard.web.app`**; `/api` → **`api-prod`**. Deployed when you **publish a GitHub Release** (**after** `api-prod` in `deploy-prod.yml`; same tag), not on every merge to `main`.
