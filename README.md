...
- **`POST /auth/token`** — exchange **email/password** (JSON body) for Firebase **ID token** and refresh token via this API (the server calls Identity Toolkit; you do not need to curl Google’s URL). Use **`id_token`** as **`Authorization: Bearer`** on admin routes (**~1h** lifetime; use **`refresh_token`** with Firebase’s refresh API or call **`POST /auth/token`** again for long jobs). Optional **`admin_only: true`** returns **`403`** unless the user has custom claim **`admin: true`**. Failed sign-in returns a **generic** **`401`** (no user enumeration). In production, set **`FIREBASE_WEB_API_KEY`** to your Firebase **Web API key** (same as in the web app config); with the Auth emulator, a placeholder key is used automatically. **Do not** enable request-body logging for this route; add **rate limiting** at your reverse proxy or API gateway.
...
- **Hosting CI builds:** Add GitHub repository secret **`VITE_FIREBASE_API_KEY`** (Firebase Console → Project settings → Web app config). Add repository variable **`PREVIEW_API_BASE_URL`** for PR previews so map/catalog API calls target staging when the preview hosting config omits Cloud Run rewrites. Set this value to the Cloud Run API origin (for example `https://api-staging-<project-number>.us-central1.run.app`) **without** `/api`. Merge and PR workflows set **`VITE_USE_AUTH_EMULATOR=false`** and the project id for **`npm run build`**.
