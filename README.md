...
9. [API integration](docs/api-integration.md) — HTTP integration for admin uploads, environmental and suitability COGs, and catalog projects  
10. [Serialized model requirements](docs/serialized-model-requirements.md) — sklearn-only pickle for **`serialized_model_file`** / SHAP (includes copy-paste brief for export agents)  
11. [Infrastructure and deployment](docs/infrastructure-and-deployment.md) — how to run and deploy the app (GCP stack, cost guardrails, what to avoid)
...
**Default (Docker Compose):** the backend sets **`FIRESTORE_EMULATOR_HOST=firebase-emulators:8085`** so the Firestore client uses the emulator. **`GOOGLE_CLOUD_PROJECT`** defaults to **`hsm-dashboard`** in **`backend_api/settings.py`** (match **`.firebaserc`**). The catalog lives in the Firestore **`models`** collection (constant **`MODELS_COLLECTION_ID`** in **`catalog_service`**). **`GET /api/models`** reads from Firestore (the frontend still calls only the API).

**`GET /api/models`** returns **`[]`** until the **`models`** collection has documents. Populate it by **seeding** from JSON (below), **export/import**, or the Emulator UI. Dev seeding lives under **`backend/scripts/`** (`seed_firestore_emulator.py`, `firestore_seed_catalog.py`), not in the API package.
...
- `GET /api/models` — list catalog entries from Firestore ([`Model`](docs/data-models.md)); **`503`** if Firestore could not be read or a document fails schema validation; **`200`** with **`[]`** if the collection is empty
- `GET /api/models/{id}` — single model (**`404`** if unknown; **`503`** when the catalog could not be loaded or failed validation)
- `POST /api/models` / `PUT /api/models/{id}` — create or update catalog entries and suitability COG (**`admin: true`** claim required; **`401`** / **`403`** otherwise). Multipart form; OpenAPI lists parts under **`/docs`** (`openapi_extra`). Writes COGs under **`LOCAL_STORAGE_ROOT`** when **`STORAGE_BACKEND=local`** (Compose), or **GCS** when **`STORAGE_BACKEND=gcs`**. Suitability COGs must be tiled **EPSG:3857** Web Mercator (not e.g. EPSG:27700 — reproject before upload; see **`docs/data-models.md`**). **`POST`** returns **409** if **`project_id` + `species` + `activity`** already exists. Validation errors use structured **`detail`**: `{ "code", "message", "context"? }`. Scripted upload flows: **`docs/api-integration.md`**.
- `GET /api/auth/me` — returns **`{ uid, email }`** from a verified Firebase **ID token**; requires **`Authorization: Bearer <token>`**; **`401`** if missing or invalid. Uses Firebase Admin **`verify_id_token`**; set **`FIREBASE_AUTH_EMULATOR_HOST`** in dev so verification targets the Auth emulator.
- **`POST /api/auth/token`** — exchange **email/password** (JSON body) for Firebase **ID token** and refresh token via this API (the server calls Identity Toolkit; you do not need to curl Google’s URL). Use **`id_token`** as **`Authorization: Bearer`** on admin routes (**~1h** lifetime; use **`refresh_token`** with Firebase’s refresh API or call **`POST /api/auth/token`** again for long jobs). Optional **`admin_only: true`** returns **`403`** unless the user has custom claim **`admin: true`**. Failed sign-in returns a **generic** **`401`** (no user enumeration). In production, set **`FIREBASE_WEB_API_KEY`** to your Firebase **Web API key** (same as in the web app config); with the Auth emulator, a placeholder key is used automatically. **Do not** enable request-body logging for this route; add **rate limiting** at your reverse proxy or API gateway.
- `GET /api/geocode/search?q=...` — public place search for the map, proxied through the backend to MapTiler so the API key stays server-side. Set **`MAPTILER_API_KEY`** in the backend environment; Compose passes it through from the host shell when present.
...
- **Dev Hosting (staging API):** **`hsm-dashboard-dev`** — **`https://hsm-dashboard-dev.web.app`** (and **`.firebaseapp.com`**); `/api` → **`api-staging`**. Deployed on **merge to `main`** after CI (see [`docs/deployment-runbook.md`](docs/deployment-runbook.md)).
- **Prod Hosting:** **`hsm-dashboard`** — **`https://hsm-dashboard.web.app`**; `/api` → **`api-prod`**. Deployed in the same **`release-deploy-prod.yml`** run as **`api-prod`** (API step first, then Hosting; same tag), not on every merge to `main`.
- **Hosting CI builds:** Add GitHub repository secret **`VITE_FIREBASE_API_KEY`** (Firebase Console → Project settings → Web app config). Add repository variable **`PREVIEW_API_BASE_URL`** for PR previews so map/catalog API calls target staging when the preview hosting config omits Cloud Run rewrites. Set this value to the Cloud Run API origin **including** `/api` (for example `https://api-staging-<project-number>.us-central1.run.app/api`). Workflows set **`VITE_USE_AUTH_EMULATOR=false`** and the project id for **`npm run build`**.

### Next steps

Catalog in Firestore and the Auth dev slice (`GET /api/auth/me`, email/password + emulator) are in place. Remaining product scope: `GET /api/models/{id}/point` polish, admin **`POST/PUT /api/models`**, **`admin` claim enforcement** on write routes, and **`/admin`** UI — see [`application-spec.md`](application-spec.md) and [**docs/admin-scope-decisions.md**](docs/admin-scope-decisions.md) (steering for [issue #9](https://github.com/MatthewJWhittle/hsm-app/issues/9): claims, local vs GCS storage, UUID ids, projects-shaped future work). The frontend keeps relative `/api/...` URLs behind Firebase Hosting rewrites to Cloud Run.
